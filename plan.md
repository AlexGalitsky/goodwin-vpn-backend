...
**Сейчас:** P0–P8 + честный Apply, dest/SNI, BT/SMTP, health/серт, живой VLESS-трафик — **в проде**. Публичная политика: `GET /privacy`. Клиент: телефоны + privacy URL / l10n / backup merge. Windows и другие платформы на паузе.
...
Клиент **не менять ради панели**, пока плоскость не отдаёт тело, которое уже проходит `subscription_parser_test` / `subscription_client_test`. Контракт `/sub` (404 ≠ пустое 200) не трогать.
...
Не начинать Hy2/TT, пока P2–P3 не коннектятся с телефона по подписке. **Это уже так** — оба протокола на Android и iPhone.

После P8 в проде: честный Apply, dest/SNI, BT/SMTP, health/серт, VLESS-квота. Очередь панели — только Hy2 traffic (2.3) и суточный сброс (3.1), если заболят. Следующая работа — клиент, не новая фаза P9.
